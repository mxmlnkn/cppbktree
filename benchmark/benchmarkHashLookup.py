...
def plot_results_to_axis(ax, timings, linestyle='-', plot_fits=True):
    ...
    # Note that absolute +- errors become relative errors when log-scaled by using Gaussian error propagation
    ...
    if plot_fits:
        ax.plot(
            element_counts,
            np.log(element_counts) / np.log(element_counts)[1] * creation_times_mean[1],
            label='O(log N)',
            zorder=20,
            color='k',
        )
...
def compare_scaling(data_files, export_name=None, print_numerical_comparison=True):
    ...
    if print_numerical_comparison and len(data_files) == 2:
        ...
        for i in range((data1.shape[1] - 3) // 3):
            print(
                f"Distance threshold {int(data1[0,3 + 3*i+0])} | {data1[-1,3 + 3*i+1]:.2e} | {data2[-1,3 + 3*i+1]:.2e} | {data1[-1,3 + 3*i+1] / data2[-1,3 + 3*i+1]:.1f}"
            )
...
if __name__ == '__main__':
    ...
    if False:
        ...
        if not os.path.exists(filePath):
            timings = benchmark_64_bit_hashes(
                element_sizes, lambda x: cppbktree.BKTree64(x, CHUNK_SIZE_IN_KILO_BINARY * 1024)
            )
            np.savetxt(filePath, timings)
        plot_results(np.genfromtxt(filePath), export_name=filePath.replace(".dat", ""))
